...
class ThriftilyManager {
  ...
  start () {
    Object.keys(this.config.clients).forEach(alias =>
      this.createThriftily(alias, this.config.clients[alias])
    )
  }
  ...
  createThriftily (alias, clientConfig) {
    ...
    const thriftily = new Thriftily(alias, {
      ...this.config.default,
      ...clientConfig
    })
    ...
    thriftily.on('error', e => {
      this.logger.error(`[egg-thriftily ${alias}] 微服务连接错误`, e)
    })

    thriftily.on('timeout', e => {
      this.logger.warn(`[egg-thriftily ${alias}] 微服务连接超时`)
    })

    thriftily.on('close', e => {
      if (
        this.config.maxAttempts !== 0 &&
        thriftily.attemptCount === this.config.maxAttempts
      ) {
        this.logger.warn(
          `[egg-thriftily ${alias}] 重连超过最大次数(${
            this.config.maxAttempts
          }次),已经停止`
        )
      } else {
        this.logger.warn(
          `[egg-thriftily ${alias}] 微服务连接断开，${
            this.config.attemptTime
          }ms后尝试重连第${thriftily.attemptCount + 1}次`
        )
        setTimeout(() => {
          thriftily.reConnect()
        }, this.config.attemptTime)
